...
export function useBattlefieldEngine(context: BattlefieldEngineContext) {
  ...
  useEffect(() => {
    ...
    const useLegacyUnitRenderer =
      ...
      unitPathRetryUntil = new Map<number, number>();
    ...
    const combatTimer = resilientSetInterval(() => {
        ...
        if (unit.targetSiteId != null) {
          ...
          if (target.team === unit.team && distance < 1.18) {
            ...
            if (!unit.path || (unit.pathIndex ?? 0) >= unit.path.length) {
              if ((unitPathRetryUntil.get(unit.id) ?? 0) > now) {
                unit.tx = unit.x;
                unit.tz = unit.z;
                continue;
              }
              ...
              if (!nextPath.length) {
                ...
                unit.tx = unit.x;
                unit.tz = unit.z;
                unitPathRetryUntil.set(unit.id, now + 900 + (unit.id % 5) * 150);
                continue;
              }
              unit.path = nextPath;
              unit.pathIndex = 0;
      ...
      if (dead.size) {
        ...
        for (const unit of g.units) {
          ...
          unitPathRetryUntil.delete(unit.id);
        ...
        if (
          ...
          fireEvent("qz_captured", () => {
            ...
            defectors.forEach((unit, index) => {
                ...
                unit.path = undefined;
                unit.pathIndex = undefined;
                const angle = (index / Math.max(1, defectors.length)) * Math.PI * 2;
    ...
    const campaignTimer = resilientSetInterval(() => {
      ...
      g.sites
        .filter((site) => site.type === "camp" && !site.destroyed)
        .forEach((camp) => {
          g.units
            .filter(
              (unit) =>
    ...
    const aiTimer = resilientSetInterval(() => {
      ...
      const aiIdleCache = new Map<number, number>(),
        aiThreatCache = new Map<number, number>(),
        computeAiIdleAt = (site: SiteState) =>
          g.units.filter(
            (unit) =>
              unit.team === aiTeam &&
              unit.siteId === site.id &&
        ...
        computeAiThreatAt = (site: SiteState) =>
          g.units.filter(
            (unit) =>
              unit.team === enemyTeam &&
              (unit.targetSiteId === site.id ||
        ...
        aiIdleAt = (site: SiteState) => {
          const cached = aiIdleCache.get(site.id);
          if (cached != null) return cached;
          const value = computeAiIdleAt(site);
          aiIdleCache.set(site.id, value);
          return value;
        },
        aiThreatAt = (site: SiteState) => {
          const cached = aiThreatCache.get(site.id);
          if (cached != null) return cached;
          const value = computeAiThreatAt(site);
          aiThreatCache.set(site.id, value);
          return value;
        },
        aiDispatch = (
          source: SiteState,
          ...
          const deployed = issueOrder(aiTeam, source, target, requested, emergency);
          if (deployed) aiIdleCache.delete(source.id);
          return deployed;
        },
        logisticsFriendlySites = g.sites.filter(
          (site) => site.team === aiTeam && !site.destroyed,
        ),
        logisticsIdleAt = aiIdleAt,
        logisticsThreatAt = aiThreatAt,
      ...
      logisticsFriendlySites
        ...
        .forEach((source) => {
          ...
          if (surplus <= 0) return;
          const deployed = aiDispatch(source, target, surplus, true);
          if (deployed) {
      ...
      for (const source of activeRoutes) {
        ...
        aiDispatch(source, target, requested, false);
      }
      if (
